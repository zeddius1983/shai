...
### Method 1: Interactive Installer (Recommended for macOS/Linux)

Shai works out of the box in any terminal, but we provide a cross-platform, idempotent **installer** to configure your environment with shai and its shell integration.
...
This launches the **Shai Installer**, which automatically installs `shai` (via `uv`) and asks if you would like to enable the hotkey-based **implicit mode**.

If you prefer to install silently without interactive prompts:
```bash
...
If you installed via the **Interactive Installer** (Method 1), you can clean up the installation and your `.zshrc` by running:
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/zeddius1983/shell-assistant/main/install.sh)" -- --uninstall
```
...
If you enabled the **implicit mode** in the Shai Installer, you can type any question directly into your terminal and press `Alt+s`:

```bash
find the largest file in this folder[Press Alt+s]
